...
class Dude:
    ...
    def fight(self, second):
        ...
        while(first_hp > 0 and second_hp > 0):
            ...
            if (self.atk_timer == timer):
                ...
                if (check < 0):
                    check = 0
                attack = self.attack_list[self.atk_index].atk_type
                mins = self.attack_list[self.atk_index].min_dmg
                maxs = self.attack_list[self.atk_index].max_dmg
                first_half = self.bonus * random.randint(0, check)
                bonus = self.get_bonus(attack, second.sp_type)
                second_half = bonus * random.randint(mins, maxs)
            ...
            if (second.atk_timer == timer):
                ...
                if (check < 0):
                    check = 0
                attack = second.attack_list[second.atk_index].atk_type
                mins = second.attack_list[second.atk_index].min_dmg
                maxs = second.attack_list[second.atk_index].max_dmg
                first_half = second.bonus * random.randint(0, check)
                bonus = second.get_bonus(attack, self.sp_type)
                second_half = bonus * random.randint(mins, maxs)
    ...
    def get_bonus(self, first, second):
        if first == 'Physical':
            ...
            if second == 'Biological':
                return 2
            elif second == 'Radioactive' or\
                    second == 'Chemical' or second == 'Technological':
                return .5
...
class DudeList():
    def __init__(self, con, cursor):
        self.con = con
        self.cursor = cursor
        self.cursor.execute('SELECT id, name, species_id, ' +
                            'plus_atk, plus_dfn, plus_hp from combatant')
        ...
        for stats in data:
            ...
            try:
                ...
                sqlString = 'SELECT type, base_atk, base_dfn, base_hp ' +\
                    'from species WHERE id = ' + str(new_dude.species_id)
        ...
        for guy in self.dude_list:
            try:
                sqlString = 'SELECT name, type, min_dmg, max_dmg, speed fro' +\
                        'm attack WHERE id in (SELECT attack_id from ' +\
                        'species_attack WHERE species_id = ' +\
                        str(guy.species_id) + ')'
                self.cursor.execute(sqlString)
    ...
    def longest_fight(self):
        subString = '(SELECT *, finish - start as dif from fight) AS new_table'
        maxQuerry = '(SELECT MAX(dif) from ' + subString + ')'
        sqlString = 'SELECT combatant_one, combatant_two, dif from ' +\
            '(SELECT *, finish - start as dif from fight) as tale' +\
            ' where dif in ' + maxQuerry
        self.cursor.execute(sqlString)
    ...
    def shortest_fight(self):
        subString = '(SELECT *, finish - start as dif from fight) AS new_table'
        maxQuerry = '(SELECT MIN(dif) from ' + subString + ')'
        sqlString = 'SELECT combatant_one, combatant_two, dif from ' +\
            '(SELECT *, finish - start as dif from fight) as tale ' +\
            'where dif in ' + maxQuerry
        self.cursor.execute(sqlString)
    ...
    def longest_total_time(self):
        ...
        for dude in self.dude_list:
            if dude.fight_time > maxs:
                results = []
                maxs = dude.fight_time
            if dude.fight_time == maxs:
                results.append(dude)
        return results
    ...
    def shortest_total_time(self):
        ...
        results = []
        for dude in self.dude_list:
            if dude.fight_time < mins:
                results = []
                mins = dude.fight_time
            if dude.fight_time == mins:
                results.append(dude)
        return results
    ...
    def most_attacks(self):
        ...
        results = []
        for dude in self.dude_list:
            if len(dude.attack_list) > maxs:
                results = []
                maxs = len(dude.attack_list)
            if len(dude.attack_list) == maxs:
                results.append(dude)
        return results
    ...
    def ultimate_showdown_of_ultimate_destiny(self):
        ...
        for x in range(0, len(self.dude_list) - 1):
            ...
            for y in range(x + 1, len(self.dude_list)):
                ...
                values = str(first.my_id) + ', ' + str(second.my_id) + ', ' +\
                    "'" + winner[0] + "'" + ', TIMESTAMP ' +\
                    "' 2016-06-10 00:00:00', TIMESTAMP '2016-06-10 " + time +\
                    "'"
